...
const kyc_workflow_service_1 = require("../services/kyc-workflow.service");
const logger_1 = require("../utils/logger");
class KYCController {
    kycService;
    constructor() {
        this.kycService = kyc_workflow_service_1.KYCWorkflowService.getInstance();
    }
    getKYCStatus = async (req, res) => {
        const userId = req.query.userId || req.body?.userId || '1f475729-c2e0-4b3d-a678-e4a4ea4d6cc0';
        const userRole = req.query.userRole || kyc_workflow_service_1.UserRole.USER;
        try {
    ...
    submitKYCForReview = async (req, res) => {
    ...
    getKYCRequirements = async (req, res) => {
        const userRole = req.query.userRole || kyc_workflow_service_1.UserRole.USER;
        try {
            logger_1.logger.info('Getting KYC requirements', { userRole });
            ...
            logger_1.logger.error('Failed to get KYC requirements', {
                userRole,
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            res.status(500).json({
    ...
    getPendingReviews = async (req, res) => {
        ...
        try {
            logger_1.logger.info('Getting pending KYC reviews', { userRole });
    ...
    approveKYC = async (req, res) => {
        ...
        const approvedBy = req.body.adminId || 'admin';
        try {
            if (!userId || !userRole) {
                res.status(400).json({
                    success: false,
    ...
    rejectKYC = async (req, res) => {
        ...
        const rejectedBy = req.body.adminId || 'admin';
        try {
    ...
    getHealthStatus = async (req, res) => {
        try {
            ...
            logger_1.logger.error('KYC health check failed', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            res.status(500).json({
                success: false,
