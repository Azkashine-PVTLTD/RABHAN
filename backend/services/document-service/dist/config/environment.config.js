...
exports.config = {
    ...
    server: {
        ...
        allowedOrigins: process.env.ALLOWED_ORIGINS || 'http://localhost:3000,http://127.0.0.1:3000,http://localhost:3001',
        maxUploadsPerHour: parseInt(process.env.MAX_UPLOADS_PER_HOUR || '50', 10),
    },
    database: {
    ...
    redis: {
        ...
        keyExpiry: parseInt(process.env.REDIS_KEY_EXPIRY || '3600', 10),
    },
    minio: {
    ...
    virusScanner: {
        ...
        scanTimeout: parseInt(process.env.VIRUS_SCAN_TIMEOUT || '60000', 10),
    ...
    ocr: {
        ...
        languages: process.env.OCR_LANGUAGES || 'eng+ara',
    ...
    sama: {
        ...
        incidentNotificationTimeoutMs: parseInt(process.env.SAMA_INCIDENT_TIMEOUT || '14400000', 10),
...
function validateConfig() {
    ...
    if (exports.config.isProduction) {
        requiredEnvVars.push('AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'SAMA_NOTIFICATION_ENDPOINT', 'SAMA_REPORTING_ENDPOINT');
    }
    const missingVars = requiredEnvVars.filter(varName => !process.env[varName]);
    if (missingVars.length > 0) {
        throw new Error(`Missing required environment variables: ${missingVars.join(', ')}`);
    }
    if (exports.config.server.maxFileSize > 104857600) {
        throw new Error('MAX_FILE_SIZE cannot exceed 100MB');
    }
    if (exports.config.validation.allowedFormats.length === 0) {
        throw new Error('ALLOWED_FORMATS must contain at least one format');
    }
    if (exports.config.sama.dataRetentionYears < 7) {
        throw new Error('SAMA_DATA_RETENTION_YEARS must be at least 7 years');
    }
    if (exports.config.encryption.keyLength < 32) {
