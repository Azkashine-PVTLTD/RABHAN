...
const updateProfileSchema = Joi.object({
  ...
  employment_status: Joi.string().valid('government', 'private', 'selfEmployed', 'student', 'retired'),
  employer_name: Joi.string().min(2).max(100),
  job_title: Joi.string().min(2).max(100),
  monthly_income: Joi.string().min(1).max(20), // Allow as string for flexible formatting
  years_employed: Joi.string().min(1).max(10),   // Allow as string for flexible formatting
