...
class EmailService {
    ...
    async sendVerificationEmail(email, token, userId) {
        try {
            const verificationLink = `${environment_config_1.config.frontend.url}/verify-email?token=${token}&userId=${userId}`;
            const emailContent = this.generateVerificationEmailHTML(verificationLink);
            const mailOptions = {
                from: environment_config_1.config.email.from,
                to: email,
                subject: 'Verify Your Email - RABHAN Solar Platform',
                html: emailContent
            };
            await this.transporter.sendMail(mailOptions);
            logger_1.logger.info(`Verification email sent to ${email} for user ${userId}`);
            return {
                success: true,
      ...
      <body>
        <div class="container">
          <div class="card">
            ...
            <a href="${environment_config_1.config.frontend.url}/dashboard" class="cta-button">
              Go to Dashboard
            </a>
     ...
     */
    async simulateEmailSend(mailOptions) {
        // In production, replace this with actual email sending logic
        logger_1.logger.info('📧 EMAIL SIMULATION:', {
            to: mailOptions.to,
        ...
        if (process.env.NODE_ENV === 'development') {
            logger_1.logger.info('Email content preview:', mailOptions.html.substring(0, 200) + '...');
        }
    }
}
