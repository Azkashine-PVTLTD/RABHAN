...
import { SimpleSolarCalculator } from './components/calculator/SimpleSolarCalculator';
...
// Authenticated UserApp wrapper with sidebar
const DashboardPage = () => {
  const [user, setUser] = useState<any>(null);
...
const ContractorDashboardPage = () => {
  ...
  React.useEffect(() => {
    ...
    if (authState.user?.role === 'USER' && location.pathname.startsWith('/contractor')) {
      navigate('/dashboard');
      return;
    }
    ...
    const unsubscribe = authService.subscribe((state) => {
      setUser(state.user);
      // Redirect users to user dashboard
      if (state.user?.role === 'USER' && location.pathname.startsWith('/contractor')) {
        navigate('/dashboard');
      }
    });
  ...
  return <ContractorApp user={user} onLogout={handleLogout} initialActiveItem={getActiveMenuFromURL()} />;
};
