...
class AuthService {
    ...
    try {
      ...
      if (loginResponse.success && loginResponse.data) {
        ...
        if (userResponse.success && userResponse.data) {
          ...
          // Try to fetch profile data from user service, but don't fail login if it fails
          try {
            ...
            if (profileResponse.success && profileResponse.data) {
              ...
              mergedUser = {
                ...userResponse.data,
                ...transformedProfile,
                id: userResponse.data.id, // Keep auth user ID
                email: userResponse.data.email, // Keep auth email
                role: userResponse.data.role, // Keep auth role
              };
            } else {
  ...
  public async lookupEmailForLogin(email: string): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const response = await apiService.request('/auth/login/email/lookup', 'POST', { email });
  ...
  public async sendLoginOTPToPhone(email: string): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const response = await apiService.request('/auth/login/email/send-otp', 'POST', { email });
      return { success: response.success, data: response.data, error: response.error };
    } catch (error: any) {
  ...
  public async verifyLoginOTP(data: { email: string; otp: string }): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  public async checkAuthStatus(): Promise<void> {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Try to fetch profile data from user service, but don't fail if it errors
        try {
          const profileResponse = await apiService.user.getProfile();
          
          if (profileResponse.success && profileResponse.data) {
            console.log('✅ Profile data fetched successfully');
            // Transform and merge profile data with auth user data
            const transformedProfile = this.transformProfileData(profileResponse.data);
            mergedUser = {
              ...response.data,
              ...transformedProfile,
              id: response.data.id, // Keep auth user ID
              email: response.data.email, // Keep auth email
              role: response.data.role, // Keep auth role
            };
          } else {
            console.log('ℹ️ No profile data found, using auth data only');
          }
